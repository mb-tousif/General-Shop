...
const FashionDescription = ({ fashion }) => {
  const adURL ="https://rukminim1.flixcart.com/lockin/774/185/images/CCO__PP_2019-07-14.png?q=50";
  const date = new Date(new Date().getTime() + 5 * 24 * 60 * 60 * 1000);
  return (
    <>
      <Typography>{fashion.title.longTitle}</Typography>
      <Typography
      ...
      <Typography style={{ display: "flex", alignItems: "center" }}>
        ...
        <Box component="span" style={{ color: "#878787" }}>
          <strike>₹{fashion.price.mrp}</strike>
        </Box>
        &nbsp;&nbsp;&nbsp;
        <Box component="span" style={{ color: "green" }}>
          {fashion.price.discount}
        </Box>
      </Typography>
      <SmallText>
        <Typography style={{ display: "flex", alignItems: "center" }}>
      ...
      <Table>
        <TableBody>
          ...
          <ColumnText>
            ...
            <TableCell>{fashion.description}</TableCell>
          </ColumnText>
        </TableBody>
      </Table>
    </>
...
export default FashionDescription;
